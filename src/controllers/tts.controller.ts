...
export class TTSController {
   ...
   */
  static async textToSpeech(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
    ...
    try {
      ...
      res.json({
        ...
        voices: voices.map(voice => ({
          key: Object.keys(PERSONA_VOICES).find(key => PERSONA_VOICES[key as keyof typeof PERSONA_VOICES].id === voice.id),
          id: voice.id,
          name: voice.name,
  ...
  static async previewVoice(req: Request, res: Response): Promise<void> {
    try {
      ...
      const sampleText = "Hi! I'm your H2Oasis recovery specialist. I'm here to help you optimize your wellness journey with personalized guidance.";
