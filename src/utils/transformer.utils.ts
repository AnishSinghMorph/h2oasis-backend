...
export class TransformerUtils {
  static transformSleepSummary(sleepSummary: any): ISleepData | null {
    ...
    if (duration.sleep_duration_seconds_int) {
      cleanData.duration_minutes = Math.round(
        duration.sleep_duration_seconds_int / 60,
      );
    }
    ...
    if (duration.light_sleep_duration_seconds_int) {
      cleanData.light_sleep_minutes = Math.round(
        duration.light_sleep_duration_seconds_int / 60,
      );
    }

    if (duration.rem_sleep_duration_seconds_int) {
      cleanData.rem_sleep_minutes = Math.round(
        duration.rem_sleep_duration_seconds_int / 60,
      );
    }

    if (duration.deep_sleep_duration_seconds_int) {
      cleanData.deep_sleep_minutes = Math.round(
        duration.deep_sleep_duration_seconds_int / 60,
      );
    }

    if (duration.time_awake_during_sleep_seconds_int) {
      cleanData.awake_minutes = Math.round(
        duration.time_awake_during_sleep_seconds_int / 60,
      );
    }

    const hrData: any = {};
    if (heartRate.hr_minimum_bpm_int)
      hrData.min_bpm = heartRate.hr_minimum_bpm_int;
    if (heartRate.hr_maximum_bpm_int)
      hrData.max_bpm = heartRate.hr_maximum_bpm_int;
    if (heartRate.hr_avg_bpm_int) hrData.avg_bpm = heartRate.hr_avg_bpm_int;
    if (heartRate.hr_resting_bpm_int)
      hrData.resting_bpm = heartRate.hr_resting_bpm_int;
    ...
    const hrvData: any = {};
    if (heartRate.hrv_avg_rmssd_float)
      hrvData.rmssd_avg_ms = heartRate.hrv_avg_rmssd_float;
    if (heartRate.hrv_avg_sdnn_float)
      hrvData.sdnn_avg_ms = heartRate.hrv_avg_sdnn_float;
    ...
    if (breathing.breaths_avg_per_min_int)
      breathingData.avg_breaths_per_min = breathing.breaths_avg_per_min_int;
    if (breathing.saturation_avg_percentage_int)
      breathingData.spo2_avg_percentage =
        breathing.saturation_avg_percentage_int;
    if (breathing.saturation_minimum_percentage_int)
      breathingData.spo2_min_percentage =
        breathing.saturation_minimum_percentage_int;
    if (breathing.saturation_maximum_percentage_int)
      breathingData.spo2_max_percentage =
        breathing.saturation_maximum_percentage_int;
  ...
  static transformPhysicalSummary(physicalSummary: any): IPhysicalData | null {
    ...
    if (calories.calories_expenditure_kcal_float) {
      cleanData.calories_kcal = Math.round(
        calories.calories_expenditure_kcal_float,
      );
    }

    if (calories.calories_net_active_kcal_float) {
      cleanData.active_calories_kcal = Math.round(
        calories.calories_net_active_kcal_float,
      );
    }

    if (calories.calories_basal_metabolic_rate_kcal_float) {
      cleanData.basal_metabolic_rate_kcal = Math.round(
        calories.calories_basal_metabolic_rate_kcal_float,
      );
    }
    ...
    if (activity.moderate_intensity_seconds_int) {
      cleanData.moderate_intensity_minutes = Math.round(
        activity.moderate_intensity_seconds_int / 60,
      );
    }

    if (activity.vigorous_intensity_seconds_int) {
      cleanData.vigorous_intensity_minutes = Math.round(
        activity.vigorous_intensity_seconds_int / 60,
      );
    }
  ...
  static transformBodySummary(bodySummary: any): IBodyData | null {
    ...
    if (bodyMetrics.weight_kg_float)
      cleanData.weight_kg = bodyMetrics.weight_kg_float;
    if (bodyMetrics.height_cm_int)
      cleanData.height_cm = bodyMetrics.height_cm_int;
    if (bodyMetrics.bmi_float)
      cleanData.bmi = Math.round(bodyMetrics.bmi_float * 100) / 100;
    if (bodyMetrics.muscle_composition_percentage_int)
      cleanData.muscle_percentage =
        bodyMetrics.muscle_composition_percentage_int;
    if (bodyMetrics.water_composition_percentage_int)
      cleanData.water_percentage = bodyMetrics.water_composition_percentage_int;
    if (bodyMetrics.bone_composition_percentage_int)
      cleanData.bone_percentage = bodyMetrics.bone_composition_percentage_int;

    const bpData: any = {};
    if (bloodPressure.blood_pressure_avg_object?.systolic_mmHg_int) {
      bpData.systolic_mmHg =
        bloodPressure.blood_pressure_avg_object.systolic_mmHg_int;
    }
    if (bloodPressure.blood_pressure_avg_object?.diastolic_mmHg_int) {
      bpData.diastolic_mmHg =
    ...
    if (bloodGlucose.blood_glucose_avg_mg_per_dL_int) {
      cleanData.blood_glucose_mg_dl =
        bloodGlucose.blood_glucose_avg_mg_per_dL_int;
    }
  ...
  static transformActivityEvent(
    ...
    return activityEvents.map((event) => {
      ...
      if (activity.activity_type_name_string)
        cleanEvent.activity_type = activity.activity_type_name_string;
      if (activity.activity_start_datetime_string)
        cleanEvent.start_time = activity.activity_start_datetime_string;
      if (activity.activity_end_datetime_string)
        cleanEvent.end_time = activity.activity_end_datetime_string;
      if (activity.activity_duration_seconds_int) {
        cleanEvent.duration_minutes = Math.round(
          activity.activity_duration_seconds_int / 60,
        );
      }

      if (calories.calories_net_active_kcal_float) {
        cleanEvent.calories_burned_kcal = Math.round(
          calories.calories_net_active_kcal_float,
        );
      }
      ...
      if (heartRate.hr_avg_bpm_int)
        cleanEvent.avg_heart_rate_bpm = heartRate.hr_avg_bpm_int;
      if (heartRate.hr_maximum_bpm_int)
        cleanEvent.max_heart_rate_bpm = heartRate.hr_maximum_bpm_int;
  ...
  static transformBodyMetricsEvent(bodyMetricsEvents: any[]): IBodyData | null {
    if (
      !bodyMetricsEvents ||
      !Array.isArray(bodyMetricsEvents) ||
      bodyMetricsEvents.length === 0
    ) {
      return null;
    }
  ...
  static extractSource(payload: any): string {
    const locations = [
      payload.sleep_health?.summary?.sleep_summary?.metadata
        ?.sources_of_data_array,
      payload.physical_health?.summary?.physical_summary?.metadata
        ?.sources_of_data_array,
      payload.body_health?.summary?.body_summary?.metadata
        ?.sources_of_data_array,
      payload.physical_health?.events?.activity_event?.[0]?.metadata
        ?.sources_of_data_array,
      payload.body_health?.events?.body_metrics_event?.[0]?.metadata
        ?.sources_of_data_array,
    ];
